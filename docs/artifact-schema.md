...
Schema: `limes-workspace-lens/tokenizer-term-map.v0.1`

Generated by:

```bash
python3 -m limes_workspace_lens build-tokenizer-term-map SPEC \
  --tokenizer TOKENIZER_ID_OR_PATH \
  --tokenizer-revision TOKENIZER_REV \
  --out tokenizer-term-map.json
```

Validated by:

```bash
python3 -m limes_workspace_lens validate-tokenizer-term-map tokenizer-term-map.json --spec SPEC
```

The term map records how audit terms and prompt-level expected workspace terms tokenize under a specific tokenizer revision. It stores raw, leading-space, and casefold variants; tokenizer IDs; decoded tokenizer pieces; and whether a variant is a single tokenizer token. `summarize-readouts --term-map tokenizer-term-map.json` uses token-id matches first, then normalized single-token text variants. The legacy normalized-string matcher is used only when no term map is supplied.

Multi-token variants are recorded as diagnostics but are not counted as phrase-level readout hits. This avoids pretending that independent top-k readout tokens prove a multi-token concept appeared as a coherent phrase.

Only public metadata is secret/path linted. Term strings and tokenizer pieces are not linted because real audit vocabularies may intentionally include strings such as `secret`, `fake`, or `token`.
