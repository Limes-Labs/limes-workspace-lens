...
Validate a gradient-attribution artifact from a real PyTorch/JAX/autograd runner:

```bash
python3 -m limes_workspace_lens validate-gradient-attribution runs/gradient-attribution.json \
  --spec examples/workspace_audit_spec.json
```

This command validates provenance, compatibility, target linkage, attribution scores, row quality flags, and prompt coverage. It does not compute gradients by itself.
...
python3 -m limes_workspace_lens validate-gradient-attribution runs/gradient-attribution.json --spec runs/spec.json
